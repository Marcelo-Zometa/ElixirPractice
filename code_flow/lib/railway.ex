...
defmodule CodeFlow.Railway do
  ...
  def validate_at_least_age({:ok, %User{age: age} = user}, cutoff_age) when
    age >= cutoff_age do
      {:ok, user}
    end
  end
  ...
  def check_name_blacklist({:ok, %User{name: name} = _user}) when
    name in ["Tom", "Tim", "Timmy"] do
  ...
  def check_name_blacklist({:error, _reason} = error), do
    error
  end
end
